...
#port considere in_port and state=metadata: matching headers are in_port + metadata
    def add_flow(self, datapath,table_miss=False):
	    ...
	    datapath.send_msg(mod)
            ...
            match = datapath.ofproto_parser.OFPMatch(in_port=1)
	    actions = [datapath.ofproto_parser.OFPActionOutput(2,0)]
            inst=[datapath.ofproto_parser.OFPInstructionState(1,12000000,2),datapath.ofproto_parser.OFPInstructionActions(datapath.ofproto.OFPIT_APPLY_ACTIONS,actions)]
	    ...
	    datapath.send_msg(mod)
            ...
            match = datapath.ofproto_parser.OFPMatch(in_port=2)
